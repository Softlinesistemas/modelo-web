...
export default function GrupoPreferenciasVinculos({ register, watch, setValue }: Props) {
  const vinculos = [
    'Social', 'Solidário', 'Ambiental', 'Cultural', 'Religioso', 
    'Esportivo', 'Educacional', 'Saúde', 'Profissional'
  ]
          ...
          {vinculos.map((vinculo) => {
            ...
            return (
              <label key={vinculo} className="flex items-center gap-2">
                <Checkbox
                  checked={checked}
                  onChange={(e) => alternarVinculo(vinculo, e.target.checked)}
                />
                <span>{vinculo}</span>
              </label>
            )
