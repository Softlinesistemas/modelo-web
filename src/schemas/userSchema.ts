...
export const otherActivitiesSchema = z.array(
  z.strictObject({
    CodAtividadeTipo: z.number().int().positive().min(1, 'Código do tipo de atividade é obrigatório'),
...
export const InterestSchema = z.strictObject({
  CodTipo: z.number().int().positive().min(1, 'Código do tipo de interesse é obrigatório'),
  CodCategoria: z.number().int().positive().min(1, 'Código da categoria de interesse é obrigatório').optional(),
  CodModalidade: z.number().int().positive().min(1, 'Código da modalidade de interesse é obrigatório').optional(),
});

export const userBasicSchema = z.object({
  Nome: z.string().min(1, 'Nome é obrigatório').max(100, 'Nome deve ter no máximo 100 caracteres'),
  Usuario: z.string().min(1, 'Usuário é obrigatório').max(100, 'Usuário deve ter no máximo 100 caracteres'), 
  ...
  Cep: z.string().min(1, 'CEP é obrigatório').max(9, 'CEP deve ter no máximo 10 caracteres').optional(),
  ...
  Privacidade: z.enum(['PUBLICO', 'PRIVADO', 'AMIGOS']).default('PUBLICO').describe('Privacidade do perfil da empresa. Pode ser PUBLICO, PRIVADO ou AMIGOS.').optional(),

  ReceberAnuncios: z.boolean({ message: 'Você deve informar se deseja receber anúncios ou não.' }),
...
export const userExtraSchema = z.strictObject({
  ...
  Apresentacao: z.string().max(3000, 'Apresentação deve ter no máximo 3000 caracteres').optional().nullable(),
  ...
  WhatsappTelegram: z.string().max(15, 'WhatsApp/Telegram deve ter no máximo 15 caracteres').optional().nullable(),
  EmailContato: z.string().email('Email inválido').max(100, 'Email deve ter no máximo 100 caracteres').optional().nullable(),
  Site: z.string().max(100, 'Site deve ter no máximo 100 caracteres').optional().nullable(),
  Instagram: z.string().max(100, 'Instagram deve ter no máximo 100 caracteres').optional().nullable(),
  Facebook: z.string().max(100, 'Facebook deve ter no máximo 100 caracteres').optional().nullable(),
  Youtube: z.string().max(100, 'Youtube deve ter no máximo 100 caracteres').optional().nullable(),
  Tiktok: z.string().max(100, 'Tiktok deve ter no máximo 100 caracteres').optional().nullable(),
  Linktree: z.string().max(100, 'Linktree deve ter no máximo 100 caracteres').optional().nullable(),

  Bioma: z.number().int().min(1, 'Código do bioma é obrigatório').optional().nullable(),
  DivisaoGeopolitica: z.number().int().min(1, 'Código da divisão geopolítica é obrigatório').optional().nullable(),
