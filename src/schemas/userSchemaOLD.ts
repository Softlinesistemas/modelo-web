...
export const userBasicSchema = z.object({
    ...
    .refine(
      async (value) => {
        try {
          ...
          const data = res.data;
          return !data.exists; // se existe, retorna false
        } catch (err) {
          console.error(err);
          return false; // falha na API = inválido
        }
      },
  ...
  AnoNascimento: z.number()
    ...
    .max(2100, 'Ano de nascimento deve estar entre 1900 e 2100'),
  ...
  ContatosApoio: z.array(createContactsSupportSchema.or(connectsSupportSchema))
